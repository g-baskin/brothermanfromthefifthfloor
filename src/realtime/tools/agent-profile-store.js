...
export function loadAgentProfile(storePath = getAgentProfileStorePath()) {
  ...
  if (!row || typeof row.value !== "string") {
    return normalizeAgentProfile(DEFAULT_AGENT_PROFILE);
  }
  try {
...
export function saveAgentProfile(profile, storePath = getAgentProfileStorePath()) {
  const db = getDatabase(storePath);
  const current = loadAgentProfile(storePath);
  const updates = profile && typeof profile === "object" ? profile : {};
  const normalized = normalizeAgentProfile({ ...current, ...updates });
